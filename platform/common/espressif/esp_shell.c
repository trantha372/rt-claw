 ...
 *
 * Shared interactive shell for Espressif platforms (ESP32-C3, S3, …).
 *
...
/*
 * Replace the entire line buffer with `text`.
 * Clears the visible line, copies text, redraws.
 */
                         ...
                         const char *text)
{
    ...
    /* Overwrite visible chars with spaces */
    for (int i = 0; i < *len; i++) {
        claw_console_write(" ", 1);
    }
    for (int i = 0; i < *len; i++) {
...
void esp_shell_loop(void)
{
    ...
    shell_register_cmd_table(s_builtin_commands,
                             SHELL_CMD_COUNT(s_builtin_commands));
    int board_cmd_count = 0;
    const shell_cmd_t *board_cmds = board_platform_commands(
                                        &board_cmd_count);
    if (board_cmds && board_cmd_count > 0) {
